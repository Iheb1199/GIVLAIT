...
<head>
  ...
  <title id="tt-pg">GIVLAIT | Élevage</title> 
  <link rel="icon" type="image/x-icon" href="images/favicone.png">
  <link rel="stylesheet" href="style.css">
  <link rel="stylesheet" href="elevage.css">
</head>
<body>
	<div>
  ...
  <nav>
      <ul id="br-nav">
        ...
        <li class="item-nav parent"><span class="it-hv"><a href="index.php#Élevage-Filières" id="item-el">Élevage</a><img id="icdp-el" class="ic-dp" src="images/dropdown.png"></span>
          <div class="fils" id="dp-el">
            <ul> 
              <li style="display: block; margin-bottom: 10px;"><a href="#para-int" id="st-el" class="item-dp" style="color: black; font-size:15px;font-weight:bold;"><img class="icr-dp" src="images/right-arrow.png">Systèmes d'Élevage<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;et Types d'Exploitation</a></li>
              <li style="display: block;"><a href="#tt-st" class="item-dp it-st" id="stel" style="color: black; font-size:15px;font-weight:bold;"><img class="icr-dp" src="images/right-arrow.png">Statistiques</a></li>
            </ul>
          </div>
        </li> 
    ...
    <h1 id="tt-el">Secteur de l'élevage</h1>
    <div id="para-int">
      L’élevage en Tunisie occupe une place importante dans l’économie tunisienne du fait qu’il contribue à raison de :
      <ul class="ls">
        <li>35-40 % du PIB agricole.</li> 
        <li>4-5 % du PIB global.</li>
      </ul>
    </div>
    <h1 id='tt-st'>Statistiques</h1> 
    <h1 class='tit'>Nombre des éleveurs</h1> 
    <?php
      $conn = new mysqli('localhost', 'root', '', 'givlait');
      mysqli_set_charset($conn,"utf8"); 
      $sql = "select * from nb_eleveurs ;"; 
      mysqli_query($conn, "SET NAMES 'utf8'"); 
      $res = mysqli_query($conn,$sql);  
      echo "<table class='tb-elv'><tr><th class='bdr'></th><th class='bv'>bovins</th><th class='ov'>ovins</th><th class='cp'>caprins</th><th id='cam'>camélidés</th></tr><tr><td id='n-elev'>Nombre d'éleveurs</td>";
      while($row = mysqli_fetch_assoc($res)) 
        echo "<td>".$row['bovins']."</td><td>".$row['ovins']."</td><td>".$row['caprins']."</td><td>".$row['camélidés']."</td></tr>";
      echo "</table>";
      $sql = "select * from source where nom_tab = 'nb_eleveurs';"; 
      mysqli_query($conn, "SET NAMES 'utf8'"); 
      $res = mysqli_query($conn,$sql); 
      ...
      mysqli_close($conn);   
    ?>
    <div id='tx-ef'>Ces effectifs se répartissent différemment sur les différentes régions du pays, cette répartition se résume dans le tableau suivant :</div>
    <h1 class='tit'>Répartition des effectifs selon les régions</h1> 
    <?php
      $conn = new mysqli('localhost', 'root', '', 'givlait');
      mysqli_set_charset($conn,"utf8"); 
      ...
      mysqli_query($conn, "SET NAMES 'utf8'"); 
      $res = mysqli_query($conn,$sql); 
      echo "<table class='tb-elv' id='tb-reg'><tr><th id='rgn'>Région</th><th class='bv'>bovins</th><th class='ov'>ovins</th><th class='cp'>caprins</th></tr>"; 
      while($row = mysqli_fetch_assoc($res)) 
        echo "<tr><td>".$row['region']."</td><td>".$row['bovins']."%</td><td>".$row['ovins']."%</td><td>".$row['caprins']."%</td></tr>"; 
      echo "</table>";
    ...
    ?>
    <section>
      <h1 id="tt-et">Les systèmes d’élevage et types d'exploitation</h1>
      <div>
        <h1 id="tit-se" class="tt-div">Les systèmes d’élevage</h1>
        <div>
          <h1 class="tt-se" id="tt-it">Élevage intensif intégré et organisé</h1>
          <div class="para-e">
            <div id="para-it">
              Ce système est caractérisé par :
              <ul class="ls">
                <li>Une alimentation basée sur le fourrage vert et l’ensilage.</li>
                <li>Un taux d’intégration de l’élevage à l’agriculture est élevé.</li>
              </ul>
              Et il :
              <ul class="ls">
                <li>Concerne l’élevage laitier.</li> 
                <li>Regroupe les grandes exploitations du nord.</li>
                <li>regroupe 20% des éleveurs laitiers.</li>
              </ul>
            </div>
            <img class="im" src="images/elit.jpg">
          </div>
        </div>
        <div>
          <h1 class="tt-se" id="tt-se">Élevage semi-intensif ou non intégré</h1>
          <div class="para-e">
            <div id="para-se">
              La particularité de ce système c’est qu’il est : 
              <ul class="ls">
                <li>Pratiqué pour l’engraissement des taurillons.</li> 
                <li>Développé dans le nord ouest.</li>
              </ul>
            </div>
            <img class="im" src="images/elsm.jpg">
          </div>
        </div>
        <div>
          <h1 class="tt-se" id="tt-hs">Élevage "hors sol"</h1>
          <div class="para-e">
            <div id="para-hs">
              Ce système est pratiqué en dehors des surfaces agricoles, et il est : 
              <ul class="ls">
                <li>Développé dans le centre du pays.</li> 
                <li>Concerne 50% des élevages laitiers.</li> 
                <li>Présente 22% de tout l’élevage bovin.</li>
              </ul>
            </div>
            <img class="im" src="images/eltr.jpg">
          </div>
        </div>
        <div>
          <h1 class="tt-se" id="tt-tr">Élevage traditionnel ou extensif</h1>
          <div class="para-e">
            <div id="para-tr">
              Dans ce type d’élevage, l’alimentation basée sur les parcours, moyennant une complémentation durant la période difficile.
            </div>
            <img class="im" src="images/elhs.jpg">
          </div>
        </div>
      </div>
      <div>
        <h1 id="tit-te" class="tt-div">Types d’exploitation</h1>
        <div class="para-e">
          <div id="para-te">
            Les différents types d’exploitation rencontrés dans l’agriculture tunisienne sont classés comme suit :
            <ul class="ls">
              <li>85 % des exploitants agricoles possèdent des superficies < 20 Ha.</li>
              <li>46 % des exploitants agricoles possèdent des superficies < 5 Ha.</li>
              <li>Les exploitations < 20 Ha couvrent moins des 40 % des surfaces cultivées dans le pays mais assurent 60 % de l’élevage car elles abritent 2/3 des bovins et plus de 50 % des petits ruminants.</li>
            </ul>
          </div>
          <img class="im" src="images/te.jpg"> 
        </div>
      </div>
    </section>
    <footer> 
    ...
    <div id="ctn-ft">  
        <div id="liens-ft" class="item-ft"> 
          ...
          <a href="#br-nav" class="slien li-pr lien">Présentation</a>
          <a href="#para-int" id="li-sy" class="slien lien">Systèmes d'élevage<br>et types d'exploitation</a> 
...
<script>
  ...
  { 
    ...
    ttpg.innerHTML = "GIVLAIT |  تربية المواشي";       
  ...
  function arabest()
  {
   ...
   function arabestt() 
   {
    ...
    ttel.innerHTML = "قطاع تربية المواشي"; 
    ttel.dir = "rtl"; 
    ttel.style.marginLeft = "1050px";
    ttel.style.width = "224px"; 
    ttel.style.fontSize = "35px";  
    const ttet = document.getElementById("tt-et"); 
    ttet.innerHTML = "نظم الثروة الحيوانية وأنواع الزراعة"; 
    ttet.dir = "rtl"; 
    ttet.style.marginRight = "90px";
    ttet.style.fontSize = "30px";
    const tise = document.getElementById("tit-se"); 
    tise.innerHTML = "نظم الثروة الحيوانية"; 
    tise.dir = "rtl"; 
    tise.style.marginRight = "90px";
    tise.style.fontSize = "27px";
    const ttit = document.getElementById("tt-it"); 
    ttit.innerHTML = "التربية المتكاملة والمنظمة المكثفة"; 
    ttit.dir = "rtl"; 
    ttit.style.marginLeft = "993px"; 
    ttit.style.width = "267px"; 
    ttit.style.fontSize = "25px";
    const ttse = document.getElementById("tt-se"); 
    ttse.innerHTML = "التربية الشبه المكثفة أو الغير المتكاملة"; 
    ttse.dir = "rtl"; 
    ttse.style.marginLeft = "950px"; 
    ttse.style.width = "302px"; 
    ttse.style.fontSize = "25px";
    const tths = document.getElementById("tt-hs"); 
    tths.innerHTML = "التربية 'خارج التربة'"; 
    tths.dir = "rtl"; 
    tths.style.marginLeft = "1080px"; 
    tths.style.width = "165px"; 
    tths.style.fontSize = "25px"; 
    const tttr = document.getElementById("tt-tr"); 
    tttr.innerHTML = "التربية التقليدية أو الواسعة النطاق"; 
    tttr.dir = "rtl"; 
    tttr.style.marginLeft = "980px"; 
    tttr.style.width = "270px"; 
    tttr.style.fontSize = "25px";
    const tite = document.getElementById("tit-te"); 
    tite.innerHTML = "أنواع المزارع"; 
    tite.dir = "rtl"; 
    tite.style.marginLeft = "980px"; 
    tite.style.width = "270px"; 
    tite.style.fontSize = "30px";
    const print = document.getElementById("para-int"); 
    print.innerHTML = "تلعب تربية الماشية في تونس دورًا مهمًا في الاقتصاد التونسي لأنها تساهم في:<ul class='ls'><li>٪35 - ٪40 للناتج المحلي الفلاحي.</li><li>٪5 - ٪4 للناتج المحلي الإجمالي.</li></ul>"; 
    print.dir = "rtl"; 
    print.style.fontWeight = "bold"; 
    print.style.fontSize = "25px";
    print.style.marginRight = "90px";
    print.style.lineHeight = "35px";
    const pre = document.getElementsByClassName("para-e"); 
    for(let i = 0 ; i < pre.length ; i++)
    {
      pre[i].style.lineHeight = "35px";
    } 
    const prit = document.getElementById("para-it"); 
    prit.innerHTML = "يتميز هذا النظام بما يلي:<ul class='ls'><li>علف يعتمد على العلف الأخضر والسيلاج.</li><li>ارتفاع معدل إدماج تربية الماشية في الفلاحة.</li></ul>و هو:<ul class='ls'><li>يتعلق الأمر بالتربية الخاصة بالألبان.</li><li>يعيد تجميع المزارع الكبيرة في الشمال.</li><li>تشمل 20٪ من مربي الأبقار.</li></ul>";
    prit.dir = "rtl";   
    prit.style.fontSize = "25px";
    prit.style.fontWeight = "bold"; 
    prit.style.paddingTop = "17px"; 
    const prtr = document.getElementById("para-tr"); 
    prtr.innerHTML = "في هذا النوع من التربية، التغذية على أساس المراعي، مع التكامل خلال الفترة الصعبة."; 
    prtr.dir = "rtl";   
    prtr.style.fontSize = "25px";
    prtr.style.fontWeight = "bold"; 
    prtr.style.paddingTop = "23px";
    const prase = document.getElementById("para-se"); 
    prase.innerHTML = "تتمثل خصوصية هذا النظام فيما يلي:<ul class='ls'><li>تمرن على تسمين عجول الثور.</li><li>تطورت في الشمال الغربي.</li></ul>"; 
    prase.dir = "rtl";   
    prase.style.fontSize = "25px";
    prase.style.fontWeight = "bold"; 
    prase.style.marginTop = "95px"; 
    const parhs = document.getElementById("para-hs"); 
    parhs.innerHTML = "يمارس هذا النظام خارج المناطق الفلاحية وهو:<ul class='ls'><li>تم تطويره في وسط البلاد.</li><li>يتعلق 50٪ من مزارع الألبان.</li><li>يمثل 22٪ من جميع تربية الماشية.</li></ul>";  
    parhs.dir = "rtl";   
    parhs.style.fontSize = "25px";
    parhs.style.fontWeight = "bold"; 
    parhs.style.marginTop = "85px";
    const prte = document.getElementById("para-te"); 
    prte.innerHTML = "تصنف مختلف أنواع الحيازة المصادفة في الفلاحة التونسية على النحو التالي:<ul class='ls'><li>٪85 من المزارعين يمتلكون أراضٍ > 20 هكتار.</li><li>٪46 من المزارعين لديهم مناطق > 5 هكتار.</li><li>تغطي الحيازات > 20 هكتار أقل من 40٪ من المناطق المزروعة في البلاد ولكنها توفر 60٪ من التربية لأنها تؤوي 2/3 من الماشية وأكثر من 50٪ من المجترات الصغيرة.</li></ul>";
    prte.dir = "rtl"; 
    prte.style.fontWeight = "bold"; 
    prte.style.fontSize = "25px";
    prte.style.marginTop = "12px"; 
    const lsi = document.getElementsByClassName("ls"); 
    for(let i = 0 ; i < lsi.length ; i++)
    {
      lsi[i].style.marginLeft = "0"; 
      lsi[i].style.marginRight = "30px";
    }
    ...
    ttst.dir = "rtl"; 
    ttst.style.fontSize = "33px"; 
    ttst.style.marginRight = "100px"; 
    const tit = document.getElementsByClassName('tit');
    tit[0].innerHTML = "عدد المزارعين"; 
    tit[1].innerHTML = "توزيع الموظفين حسب المنطقة";  
    for(let i = 0 ; i < tit.length ; i++)  
    {
      tit[i].dir = "rtl"; 
      tit[i].style.fontSize = "27px"; 
      tit[i].style.marginRight = "130px"; 
    }
    const txef = document.getElementById('tx-ef'); 
    txef.innerHTML = "توزع هذه الأرقام بشكل مختلف على مختلف مناطق البلد، ويرد موجز لهذا التوزيع في الجدول التالي:"; 
    txef.dir = "rtl";
    txef.style.fontWeight = "bold"; 
    txef.style.fontSize = "25px"; 
    txef.style.marginRight = "135px"; 
    const sc = document.getElementsByClassName('sc'); 
    for(let i = 0 ; i < sc.length ; i++)
    {
        sc[i].dir = "rtl"; 
        sc[i].style.marginRight = "150px"; 
    } 
    ...
    const tbelv = document.getElementsByClassName('tb-elv'); 
    for(let i = 0 ; i < tbelv.length ; i++) 
    {
      tbelv[i].style.marginLeft= "792px";
    }
    const tbreg = document.getElementById('tb-reg'); 
    tbreg.style.marginLeft = "950px";
    const rgn = document.getElementById('rgn'); 
    rgn.innerHTML = "منطقة"; 
    const cam = document.getElementById('cam'); 
    cam.innerHTML = "الإبل";
    const nl = document.getElementById('n-elev'); 
    nl.innerHTML = "عدد المربين";
    nl.style.fontWeight = "bold";
    const td = document.getElementsByTagName('td'); 
    for(let i = 0 ; i < td.length ; i++)
      {
        if (td[i].innerHTML =='nord')
        { 
          td[i].innerHTML = 'شمال'; 
          td[i].style.fontWeight = "bold"; 
        }
        else if (td[i].innerHTML =='centre')
        { 
          td[i].innerHTML = 'وسط';
          td[i].style.fontWeight = "bold";
        } 
        else if (td[i].innerHTML =='sud')
        { 
          td[i].innerHTML = 'جنوب';
          td[i].style.fontWeight = "bold";
        } 
      } 
    const bv = document.getElementsByClassName('bv');
    const ov = document.getElementsByClassName('ov'); 
    const cp = document.getElementsByClassName('cp'); 
    for(let i = 0 ; i < 2 ; i++) 
    {
      bv[i].innerHTML = "بقرة"; 
      bv[i].style.fontWeight = "bold"; 
      ov[i].innerHTML = "الأغنام"; 
      ov[i].style.fontWeight = "bold";
      cp[i].innerHTML = "الماعز";
      cp[i].style.fontWeight = "bold"; 
    }
  }
  if (sessionStorage.getItem("slp") !== null && sessionStorage.getItem("pagel") === 'oui') 
      window.scrollTo(0, parseInt(sessionStorage.getItem("slp")));
  else if (sessionStorage.getItem("slp") !== null) 
      sessionStorage.setItem("slp", null);
  sessionStorage.setItem("pagel", "oui"); 
  window.addEventListener("scroll", function(){
      const scrollPosition = window.scrollY || window.pageYOffset;  
      sessionStorage.setItem('slp', scrollPosition.toString());
    })
  ...
  sessionStorage.setItem("pagrg", "non");
    document.getElementById('im-fr').addEventListener('click', function(){
      ...
      window.location.href = "elevage.php";
